...
/**
 * Trida pro praci se live sazkami Hokej
 *
 * 
 * <code>
 * 
...
class LiveSport1011 extends LiveSport{
...
*/
  protected function UpdateAllInfo(){
  	...
  	if(!isset($_POST['score_home']) || !isset($_POST['t_1_h']) || !isset($_POST['t_1_a']) || !isset($_POST['t_2_h']) || !isset($_POST['t_2_a']) || !isset($_POST['t_3_h']) || !isset($_POST['t_3_a'])   || !isset($_POST['l_minute']) || !isset($_POST['start_at']) || !isset($_POST['score_away']))
      ...
      if (PEAR::isError($res))  {$this->error('Nepodařilo se získat score');}
  	  ...
  	  $sth = $this->dbGame->prepare("update  live_".$this->event_data['l_sport_id']." set score_home=?,score_away=?,tretina_1_home=?,tretina_1_away=?,tretina_2_home=?,tretina_2_away=?,tretina_3_home=?,tretina_3_away=? where event_id=?");
      if (PEAR::isError($sth))  {$this->error('Nepodařilo se získat score');}
      $res =& $this->dbGame->execute($sth,array($_POST['score_home'],$_POST['score_away'],$_POST['t_1_h'],$_POST['t_1_a'],$_POST['t_2_h'],$_POST['t_2_a'],$_POST['t_3_h'],$_POST['t_3_a'],$this->event_data['event_id']));
      if (PEAR::isError($res))  {$this->error('Nepodařilo se získat score');}
 ...
 */
  protected  function ReturnPeriodButton(){
   ...
   $this->json['period_b'] = '	
  	 <input type="button" value="1. třetina" '.($this->event_data['stav']==5?'style="color:red"':'').' onclick="SetPeriod(5);$(\'.b_state\').css(\'color\',\'white\');this.style.color=\'red\';" class="b_state" />
     <input type="button" value="2. třetina" '.($this->event_data['stav']==6?'style="color:red"':'').' onclick="SetPeriod(6);$(\'.b_state\').css(\'color\',\'white\');this.style.color=\'red\';" class="b_state" />
     <input type="button" value="3. třetina" '.($this->event_data['stav']==7?'style="color:red"':'').' onclick="SetPeriod(7);$(\'.b_state\').css(\'color\',\'white\');this.style.color=\'red\';" class="b_state" />
  ...
  protected  function ReturnPeriodScore(){
    ...
    if (PEAR::isError($res))  {$this->error('Nepodařilo se získat score');}
     ...
     if ($row =& $res->fetchRow()){
     	...
     	$this->json['live_period'] = '<tr><td class="live_stav_scores_th">1. třetina</td><td><input type="text" id="t_1_h" class="score_input" value="'.$row['tretina_1_home'].'" />:
                                             <input type="text" id="t_1_a" class="score_input" value="'.$row['tretina_1_away'].'" /></td></tr>
            <tr><td class="live_stav_scores_th">2. třetina</td><td><input type="text" id="t_2_h" class="score_input" value="'.$row['tretina_2_home'].'" />:
                                             <input type="text" id="t_2_a" class="score_input" value="'.$row['tretina_2_away'].'" /></td></tr>
            <tr><td class="live_stav_scores_th">3. třetina</td><td><input type="text" id="t_3_h" class="score_input" value="'.$row['tretina_3_home'].'" />:
                                             <input type="text" id="t_3_a" class="score_input" value="'.$row['tretina_3_away'].'" /></td></tr>';
     	$this->json['js_dynamic'] .= 'periodAr = new Array("t_1_h","t_1_a","t_2_h","t_2_a","t_3_h","t_3_a");';
     }
