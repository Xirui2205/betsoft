...
/**
 * Trida pro praci se live sazkami Hazena
 *
 * 
 * <code>
 * 
...
class LiveSport1020 extends LiveSport{
...
*/
  protected function UpdateAllInfo(){
  	...
  	$this->error('Nepodařilo se aktualizovat informace');
  	  ...
  	  $sth = $this->dbGame->prepare("select first_half_home,first_half_away from  live_".$this->event_data['l_sport_id']." where event_id=?");
      ...
      if ($row =& $res->fetchRow()){
      	...
      	$f_h =  $row['first_half_home'];
      	$f_a =  $row['first_half_away'];

      	
      }
      
      if($this->event_data['stav'] == LIVE_2_HALF){
      	
      	$_POST['s_h_h'] = $_POST['score_home'] - $row['first_half_home'];
      	$_POST['s_h_a'] = $_POST['score_away'] - $row['first_half_away'];
      	
      }
      else{
      	
      	$_POST['f_h_h']  = $_POST['score_home'];
      	$_POST['f_h_a'] = $_POST['score_away'];
 ...
 */
  protected  function ReturnPeriodButton(){
   ...
   $this->json['period_b'] = '	
  	 <input type="button" value="1. poločas" '.($this->event_data['stav']==3?'style="color:red"':'').' onclick="SetPeriod(3);$(\'.b_state\').css(\'color\',\'white\');this.style.color=\'red\';" class="b_state" />
     <input type="button" value="2. poločas" '.($this->event_data['stav']==4?'style="color:red"':'').' onclick="SetPeriod(4);$(\'.b_state\').css(\'color\',\'white\');this.style.color=\'red\';" class="b_state" />
 ...
 */
  protected  function ReturnPeriodScore(){
  	...
  	$sth = $this->dbGame->prepare("select first_half_home,first_half_away,second_half_home,second_half_away,yellow_card_home,yellow_card_away,red_card_home,red_card_away from  live_".$this->event_data['l_sport_id']." where event_id=?");
    if (PEAR::isError($sth))  {$this->error('Nepodařilo se získat score');}
    $res =& $this->dbGame->execute($sth,array($this->event_data['event_id']));
    if (PEAR::isError($res))  {$this->error('Nepodařilo se získat score');}
     ...
     if ($row =& $res->fetchRow()){
     	...
     	$this->json['live_period'] = '<tr><td class="live_stav_scores_th">1. polocas</td><td><input type="text" id="f_h_h" class="score_input" value="'.$row['first_half_home'].'" />:<input type="text" id="f_h_a" class="score_input" value="'.$row['first_half_away'].'" /></td></tr>
                                      <tr><td class="live_stav_scores_th">2. polocas</td><td><input type="text" id="s_h_h" class="score_input" value="'.$row['second_half_home'].'" />:<input type="text" id="s_h_a" class="score_input" value="'.$row['second_half_away'].'" /></td></tr>
                                      <tr><td class="live_stav_scores_th">Žluté karty</td><td><input type="text" id="y_c_h" class="score_input" value="'.$row['yellow_card_home'].'" />:<input type="text" id="y_c_a" class="score_input" value="'.$row['yellow_card_away'].'" /></td></tr>
                                      <tr><td class="live_stav_scores_th">Červené karty</td><td><input type="text" id="r_c_h" class="score_input" value="'.$row['red_card_home'].'" />:<input type="text" id="r_c_a" class="score_input" value="'.$row['red_card_away'].'" /></td></tr>
';
     	$this->json['js_dynamic'] .= 'periodAr = new Array("f_h_h","f_h_a","s_h_h","s_h_a","y_c_h","y_c_a","r_c_h","r_c_a");';
     }
