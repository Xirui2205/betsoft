...
/**
 * Trida pro praci se live sazkami Tenis
 *
 *
 * <code>
 *
...
class LiveSport1003 extends LiveSport{
...
*/
  public function __construct($act=NULL,$ed=NULL,$db=null,$dbGame=null){
  	...
  	if(!is_object($dbGame) || !is_object($db) || $ed == NULL || $act == NULL || $act == NULL)
      $this->error('Nepodařilo se inicializovat promenne');
...
/**
* Nastavuje pocet setu
*/
  protected function SetNUm(){
    if(isset($_POST['set_num']) && ctype_digit($_POST['set_num'])){
      $sth = $this->dbGame->prepare("UPDATE live_1003 SET set_num=? WHERE event_id=?");
      if (PEAR::isError($sth))  {$this->error('Nepodařilo se získat score');}
      $res =& $this->dbGame->execute($sth,array($_POST['set_num'],$this->event_data['event_id']));
...
* Nastavuje tiebreak
*/
  protected function SetTiebrek(){

    if(isset($_POST['tiebreak']) && ctype_digit($_POST['tiebreak'])){

      $sth = $this->dbGame->prepare("UPDATE live_1003 SET tiebreak=? WHERE event_id=?");
      if (PEAR::isError($sth))  {$this->error('Nepodařilo se získat score');}
      $res =& $this->dbGame->execute($sth,array($_POST['tiebreak'],$this->event_data['event_id']));
      if (PEAR::isError($res))  {$this->error('Nepodařilo se získat score');}
    }
  }
...
*/
  protected function FirstService(){
    ...
    if(isset($_POST['first_service'])){
      $sth = $this->dbGame->prepare("UPDATE live_1003 SET first_service=? WHERE event_id=?");
      if (PEAR::isError($sth))  {$this->error('Nepodařilo se získat score');}
...
/**
* Nastavuje aktualni kurt
*/
  protected function SetCourt(){

    if(isset($_GET['court'])){
      $sth = $this->dbGame->prepare("UPDATE live_1003 SET court=? WHERE event_id=?");
      if (PEAR::isError($sth))  {$this->error('Nepodařilo se získat score');}

      $res =& $this->dbGame->execute($sth,array($_GET['court'],$this->event_data['event_id']));
      if (PEAR::isError($res))  {$this->error('Nepodařilo se získat score');}
    }
  }
...
*/
  protected function UpdateAllInfo(){
      ...
      if (PEAR::isError($res))  {$this->error('Nepodařilo se získat score');}
  	  ...
  	  $sth = $this->dbGame->prepare("update  live_".$this->event_data['l_sport_id']." set score_home=?,score_away=?,set_1_home=?,set_1_away=?,set_2_home=?,set_2_away=?,set_3_home=?,set_3_away=?,set_4_home=?,set_4_away=?,set_5_home=?,set_5_away=? where event_id=?");
      if (PEAR::isError($sth))  {$this->error('Nepodařilo se získat score');}
      $res =& $this->dbGame->execute($sth,array($_POST['score_home'],$_POST['score_away'],$_POST['set_1_h'],$_POST['set_1_a'],$_POST['set_2_h'],$_POST['set_2_a'],$_POST['set_3_h'],$_POST['set_3_a'],$_POST['set_4_h'],$_POST['set_4_a'],$_POST['set_5_h'],$_POST['set_5_a'],$this->event_data['event_id']));
      if (PEAR::isError($res))  {$this->error('Nepodařilo se získat score');}
