...
if ( isset($_POST["sum_castka"]) ) {
	$sql = "select u.user_id, u.jmeno,u.prijmeni, sum(castka) as saz_castka from ticket as t 
				INNER JOIN `vic_main`.`uzivatel` AS `u` ON t.user_id = u.user_id 
				INNER JOIN `vic_admin`.`branch` AS `b` ON u.branch_id = b.id 
				WHERE u.anonymous!=1 AND u.block=0 AND zalozen>='" . $od . "' AND zalozen<='" . $do . "' GROUP BY u.user_id  ORDER BY saz_castka";
} else {
	$sql = "select u.user_id, u.jmeno,u.prijmeni, castka as saz_castka from ticket as t 
				INNER JOIN `vic_main`.`uzivatel` AS `u` ON t.user_id = u.user_id 
				INNER JOIN `vic_admin`.`branch` AS `b` ON u.branch_id = b.id 
				WHERE u.anonymous!=1 AND u.block=0 AND zalozen>='" . $od . "' AND zalozen<='" . $do . "' ORDER BY saz_castka";
}
...
foreach ($usersTickets as $ticket) {
	switch ($ticket["saz_castka"]) {
	    case ($ticket["saz_castka"] <= 100): 
	    	$do100++;
	    	$aUsers[100][]	 = $ticket;
	    	$aUsers[users100][$ticket["user_id"]] = $ticket["jmeno"] . " " . $ticket["prijmeni"];
	    	break;
	    case ($ticket["saz_castka"] > 100 && $ticket["saz_castka"] <= 500):
	    	$do500++;
	    	$aUsers[500][]	 = $ticket;
	    	$aUsers[users500][$ticket["user_id"]] = $ticket["jmeno"] . " " . $ticket["prijmeni"];
	    	break;
	    case ($ticket["saz_castka"] > 500 && $ticket["saz_castka"] <= 1000): 
	    	$do1000++;
	    	$aUsers[1000][]	 = $ticket;
	    	$aUsers[users1000][$ticket["user_id"]] = $ticket["jmeno"] . " " . $ticket["prijmeni"];
	    	break;
	    case ($ticket["saz_castka"] > 1000 && $ticket["saz_castka"] <= 5000): 
	    	$do5000++;
	    	$aUsers[5000][]	 = $ticket;
	    	$aUsers[users5000][$ticket["user_id"]] = $ticket["jmeno"] . " " . $ticket["prijmeni"];
	    	break;
	    case ($ticket["saz_castka"] > 5000):
	    	$nad5000++;
	    	$aUsers[nad5000][]	 = $ticket;
	    	$aUsers[usersNad5000][$ticket["user_id"]] = $ticket["jmeno"] . " " . $ticket["prijmeni"];
	    	break;
	}
}
...
<form action="<?php echo $_SERVER["PHP_SELF"]?>" method="post" enctype="multipart/form-data">
	od: <input type="text" size="10" name="od" value="<?=$od?>" />
	do: <input type="text" size="10" name="do" value="<?=$do?>" />
	<input type="checkbox" name="detail" value="0" <?php if(isset($_POST["detail"])) { echo "checked"; } ?> />ticket detail
	<input type="checkbox" name="user_detail" value="0" <?php if(isset($_POST["user_detail"])) { echo "checked"; } ?> />user detail
	<input type="checkbox" name="sum_castka" value="0" <?php if(isset($_POST["sum_castka"])) { echo "checked"; } ?> />summary castka
	<input type="submit" name="zobraz" value="zobraz" />
</form>

<?php
if ( isset($_POST["detail"]) ) {
	echo "do 100 Kč: " . count($aUsers[users100]) . " hráčů<br/>";
	...
	echo "do 500 Kč: " . count($aUsers[users500]) . " hráčů<br/>";
	foreach ($aUsers[500] as $usr) {
		echo '<div style="padding-left: 50px; font-size: 10px;">';
		echo $usr["user_id"]. " | " . $usr["saz_castka"] . " | " . $usr["jmeno"] . " " . $usr["prijmeni"] . "<br/>";
		echo '</div>';
	}
	echo "do 1000 Kč: " . count($aUsers[users1000]) . " hráčů<br/>";
	foreach ($aUsers[1000] as $usr) {
		echo '<div style="padding-left: 50px; font-size: 10px;">';
		echo $usr["user_id"]. " | " . $usr["saz_castka"] . " | " . $usr["jmeno"] . " " . $usr["prijmeni"] . "<br/>";
		echo '</div>';
	}
	echo "do 5000 Kč: " . count($aUsers[users5000]) . " hráčů<br/>";
	foreach ($aUsers[5000] as $usr) {
		echo '<div style="padding-left: 50px; font-size: 10px;">';
		echo $usr["user_id"]. " | " . $usr["saz_castka"] . " | " . $usr["jmeno"] . " " . $usr["prijmeni"] . "<br/>";
		echo '</div>';
	}
	echo "nad 5000 Kč: " . count($aUsers[usersNad5000]) . " hráčů<br/>";
	foreach ($aUsers[nad5000] as $usr) {
		echo '<div style="padding-left: 50px; font-size: 10px;">';
		echo $usr["user_id"]. " | " . $usr["saz_castka"] . " | " . $usr["jmeno"] . " " . $usr["prijmeni"] . "<br/>";
		echo '</div>';
	}
} elseif (  isset($_POST["user_detail"]) ) {
	echo "do 100 Kč: " . count($aUsers[users100]) . " hráčů<br/>";
	foreach ($aUsers[users100] as $usr) {
		echo '<div style="padding-left: 50px; font-size: 10px;">';
		echo $usr . "<br/>";
		echo '</div>';
	}
	echo "do 500 Kč: " . count($aUsers[users500]) . " hráčů<br/>";
	foreach ($aUsers[users500] as $usr) {
		echo '<div style="padding-left: 50px; font-size: 10px;">';
		echo $usr . "<br/>";
		echo '</div>';
	}
	echo "do 1000 Kč: " . count($aUsers[users1000]) . " hráčů<br/>";
	foreach ($aUsers[users1000] as $usr) {
		echo '<div style="padding-left: 50px; font-size: 10px;">';
		echo $usr . "<br/>";
		echo '</div>';
	}
	echo "do 5000 Kč: " . count($aUsers[users5000]) . " hráčů<br/>";
	foreach ($aUsers[users5000] as $usr) {
		echo '<div style="padding-left: 50px; font-size: 10px;">';
		echo $usr . "<br/>";
		echo '</div>';
	}
	echo "nad 5000 Kč: " . count($aUsers[usersNad5000]) . " hráčů<br/>";
	foreach ($aUsers[usersNad5000] as $usr) {
		echo '<div style="padding-left: 50px; font-size: 10px;">';
		echo $usr . "<br/>";
		echo '</div>';
	}
