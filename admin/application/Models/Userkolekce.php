...
// just a copy of /admin/include/class/class.UserKolekce.php Its here as
// we do not have enough time to migrate it properly. It has been edited somewhat.

class Models_Userkolekce extends Template{
 ...
 */
  ...
  public function _addTemplateMessage($nazev, $telo){
    $telo = Help::slash($telo);
    $nazev = Help::slash($nazev);
    $sql = "INSERT INTO poznamky_template (predmet, telo) VALUES ('$nazev', '$telo')";
    if($this->dbGame->query($sql))
		return true;
	else
		return false;
  }

  public function _deleteTemplateMessage($id){
    $id = intval($id);
  ...
  private function _deleteMessage($id){
  	$omheslo = Help::cryptPass($_POST['s_heslo']);
 	$sql = "SELECT
			*
			FROM admin a
			WHERE
				a.`admin_id` = " . intval($_SESSION['user_id']) . " AND
			    CONCAT(';', a.`klic`, ';') LIKE '%;1001;%' AND
			    a.heslo = '$omheslo'";
...
//return 'dd';
     return $this->ChangeBalance($_REQUEST['user_id']);
 ...
 */
 ...
 if(isset($_POST['play_block']) ) $block = 1;
         ...
         if(DB::isError($res)) throw new ExHandler($sql.'<br>Nepodarilo se provest dotaz: update uzivatele',"admin_ex_db");
     	 ...
     	 $this->dbGame->query('insert into uzivatel_poznamka (user_id,admin_id,text,datum) values('.$user_id.','.$_SESSION['user_id'].',"'.($block ==1?"Zamezení hraní":"Odblokování hraní").'","'. date('Y-m-d H:i:s') .'")');
		...
		It6_Log::info(
			"User '%id%' has been '%status%' to play.",
			It6_Log::TAG_ADMIN_OPERATION,
			array(
				'id'			=> $user_id,
				'status'	=> (isset($_POST['play_block'])?'forbiden':'allowed')
			)
		);
 ...
 */
 private function  zrusitDeposit($user_id){
       ...
       if( isset($_POST['kod_bonus'])){
         ...
         $sql = "select * from admin where admin_id=".intval($_SESSION['user_id']);
         $res =& $this->db->query($sql);
         if(DB::isError($res)) throw new ExHandler($sql.'<br>Nepodarilo se provest dotaz: update uzivatele',"admin_ex_db");
...
//           $this->vrat .= "<div class=\"okmsg\">Bonus byl smazán</div><br />";
		...
		It6_Log::info(
			"Bonus for user '%id%' was removed (from: '%from%' to: '%to%')",
			It6_Log::TAG_ADMIN_OPERATION,
			array(
				'id'	=> $user_id,
				'from'	=> $_POST['akce_od'],
 ...
 * @return void
 */
 public function  ChangeBalance($user_id){
       ...
       if(isset($_POST['zustatek']) && isset($_POST['kod'])){
         ...
         $sql = "select * from admin where admin_id=".intval($user_id);
         $res =& $this->db->query($sql);
         if(DB::isError($res)) throw new ExHandler($sql.'<br>Nepodarilo se provest dotaz: update uzivatele',"admin_ex_db");
...
//           $this->vrat .= "<div class=\"okmsg\">Zůstatek byl editován</div><br />";
		...
		It6_Log::info(
			"Balance for user '%id%' was changed (old balance: '%oldBalance%' new balance: '%newBalance%')",
			It6_Log::TAG_ADMIN_OPERATION,
			array(
				'id'	=> $user_id,
				'oldBalance'	=> $old_zustatek,
				'newBalance'	=> floatval($_POST['zustatek'])
			)
		);
 ...
 * @return void
 */
 public function  CreatePoznamkaBook($userId){

  $status = true;

	if(isset($_POST['book_info_value']) && (mb_strlen($_POST['book_info_value']) > 2000 || mb_strlen($_POST['book_info_value']) < 1)) {
		return "<div class=\"errormsg\"><strong>Text</strong> musí mít  1-2000 znaků</div><br />";
		$status = false;
	}
  ...
  if($status){
     ...
     $sql = "update uzivatel set book_info='".Help::Slash($_POST['book_info_value'])."' where user_id=".intval($userId);
     $res =& $this->dbGame->query($sql);
     if(DB::isError($res)) throw new ExHandler($sql.'<br>Nepodarilo se provest dotaz: update uzivatele',"admin_ex_db");
...
//     $this->vrat .
		...
		It6_Log::info(
			"Bookmaker note for user '%id%' was added.",
			It6_Log::TAG_ADMIN_OPERATION,
			array('id' => $userId)
		);
 ...
 * @return void
 */
 public function  CreatePoznamka($userId){

  $status = true;

	if(isset($_POST['poznamka_text']) && (mb_strlen($_POST['poznamka_text']) > 2000 || mb_strlen($_POST['poznamka_text']) < 1)) {
		return "<div class=\"errormsg\"><strong>Text</strong> musí mít  1-2000 znaků</div><br />";
		$status = false;
	}
  ...
  if($status){
	$adminId = Zend_Registry::get('acl')->getIdentity(It6_Acl::IDNAME_ADMIN);
     $sql = "insert into uzivatel_poznamka (user_id, text, admin_id, datum) values(".intval($userId).",'".Help::Slash($_POST['poznamka_text'])."',".$adminId.",'".date("Y-m-d H:i:s")."')";
     $res =& $this->dbGame->query($sql);
     if(DB::isError($res)) throw new ExHandler($sql.'<br>Nepodarilo se provest dotaz: update uzivatele',"admin_ex_db");
...
//     $this->vrat .= "<div class=\"okmsg\">Uživateli byly  (".(isset($_POST['validuser'])?"povoleny":"zakázány").") výbiry</div><br />";
	...
	$this->messages[] = 'Uživateli byly  ('.(isset($_POST['validuser'])?"povoleny":"zakázány").') výběry';
		...
		It6_Log::info(
			"Cash withdrawal for user '%id%' was '%privilage%'",
			It6_Log::TAG_ADMIN_OPERATION,
			array(
				'id'	=> $user_id,
				'privilage'	=> (isset($_POST['validuser'])?"allowed":"forbiden")
			)
		);
 }
...
//     $this->vrat .= "<div class=\"okmsg\">Uživatel byl úspěšně (".(isset($_POST['povolit'])?"povolen":"zakázán").")</div><br />";
	$this->messages[] = I18n::tr("Uživatel ID {1} byl ".(isset($_POST['povolit'])?"povolen":"zakázán").".",$user_id);
		...
		It6_Log::info(
			...
			array(
				'id'	=> $user_id,
				'privilage'	=> ($_POST['povolit'])?"allowed":"banned"
			)
		);
 }
   ...
   $status = true;
      ...
      if(isset($_POST['nick']) && isset($_POST['email']) && mb_strlen(trim($_POST['email'])) > 0 && mb_strlen(trim($_POST['nick'])) > 0){
       ...
       $sql = "insert into uzivatel(jmeno,prijmeni,nick,heslo,email,mena_id,zeme_id,misto,pohlavi,datum_narozeni,ulice,psc,mobil,newsletter,datum_registrace,lang_id)
         ...
         ".(isset($_POST['newsletter'])?1:0).",'".date("Y-m-d H:i:s")."',".intval($_POST['lang_id']).")";
        $res =& $this->dbGame->query($sql);
        if(DB::isError($res)) throw new ExHandler($sql.'<br>Nepodarilo se provest dotaz: vlozeni noveho uzivatele',"admin_ex_db");
...
//                $this->vrat .= "<div class=\"okmsg\">Block se podarilo zmenit</div><br />";
				...
				It6_Log::info(
					"User '%id%' was ",
					It6_Log::TAG_ADMIN_OPERATION,
					array('id' => (intval($_POST['block'])?'blocked':'unblocked'))
				);
            }
         }
      }
 ...
 */
 final private function showDetail($user_id){
    ...
    if($row =& $res9->fetchRow()){
     ...
     $uzivatel = array("zustatek"=>$row['zustatek'],"dluh"=>$row['dluh']);
    ...
    if(isset($_POST['newpass'])){
     ...
     $sql = "
		...
		INNER JOIN banks b
			ON uba.bank_id = b.bank_id
		WHERE uba.user_id=".$user_id;
     $res10 =& $this->dbGame->query($sql);
     if(DB::isError($res10)) throw new ExHandler($sql.'<br>Nepodarilo se provest dotaz: vyber z tabulky user_bank_accounts',"admin_ex_db");
...
*/
	...
	$this->vrat .= '
		...
		<ul class="idTabs" style="display:block">
			...
			<li><a href="?section=76&user='.$user_id.(isset($_GET['superb'])?"&superb=1":"").'">Sázky statistika</a></li>
			<li><a href="?section=88&user='.$user_id.(isset($_GET['superb'])?"&superb=1":"").'">Saldo</a></li>
...
//	$this->vrat .= "<div id='inner'></div>";
	...
	*/
	private function shoda($uid = 0){
		...
		$sth3 = $this->dbGame->prepare("
			SELECT user_id, nick, xforward, persistent_id, ts
				FROM user_tracking
				WHERE user_id = ?
		");
		...
		while ($row =& $res3->fetchRow()){
			...
			$sql = "
				SELECT a.user_id, a.nick, a.email
					FROM uzivatel a
					INNER JOIN user_tracking b
						ON a.user_id = b.user_id
					WHERE a.user_id <> ".$row['user_id']."
						AND (b.xforward NOT IN('',".implode(",",$ipd).")
						AND b.xforward = '".$row['xforward']."')
			";
			...
			while ($row2 =& $res->fetchRow()){
				$ipv[$row2['user_id']]['id']	= '<p><a href="?section=40&user_id='.$row2['user_id'].(isset($_GET['superb'])?'&superb=1':'').'" target="_blank"> '.$row2['user_id'].'  </a></p>';
		...
		$sth = $this->dbGame->prepare("
			SELECT a.user_id, a.nick, b.email
				FROM uzivatel b
				INNER JOIN user_tracking a
					ON b.user_id = a.user_id
				WHERE a.persistent_id IN (".implode(',',$user['per']).")
					AND a.user_id <> ?
					AND a.xforward NOT IN('',".implode(",",$ipd).")
				GROUP BY a.user_id
		");
		...
		while ($row =& $res3->fetchRow()){
			if(!isset($ipv[$row['user_id']])){
				$ipv[$row['user_id']]['id']		= '<p><a href="?section=40&user_id='.$row['user_id'].(isset($_GET['superb'])?'&superb=1':'').'" target="_blank"> '.$row['user_id'].'  </a></p>';
		...
		ksort($ipv);
		if($status)
			$vrat .= '<strong>Počet shod: </strong>'. count($ipv);
        ...
        foreach($ipv as $k=>$h){
			$win = $roz = 0;
			$sth3 = $this->dbGame->prepare("
				SELECT fn_currency_user2central(a.user_id, b.zustatek) AS zustatek, a.vyhernost,round(
					(
						(
							(
								(SELECT COALESCE(SUM(t.acctrans_amount),0)
									FROM accTrans t
									WHERE t.acctrans_sign = 1
										AND t.acctrans_USR = ?
								)
								-
								(SELECT COALESCE(SUM(l.acctrans_amount),0)
									FROM accTrans l
									WHERE l.acctrans_sign = 2
										AND l.acctrans_USR = ?
								)
								-
								b.zustatek
							...
							/
							(SELECT e.kurz
								FROM game.kurzmena e
								WHERE e.mena_id = a.mena_id
									AND e.platny_od <= now()
									AND e.platny_do >= now()
							)
						)
			...
			$vrat .= "
				<tr >
					...
					<td>".$h['nick']."</td>
					<td>".$h['email']."</td>
					<td>".(isset($h['PC'])?'ANO':'NE')."</td>
					<td>".(isset($h['IP'])?'ANO':'NE')."</td>
					<td>". $win ." %</td>
					<td>".$zus." </td>
				</tr>
			";
		...
		$vrat .= '</table>';
		if($status)
			$vrat .= '<hr /><p><strong>Návštěvy</strong></p>';
 ...
 */
	...
	private function getAdminName() {
		return Zend_Registry::get('zdb_admin')->select()
			->from("admin",array("nick"))
