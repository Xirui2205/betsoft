...
class DataSettingsController extends controllers_ParamsAbstractController {

	const MODE_DEFAULT = 1;
	const MODE_BOOKMAKER = 2;

	protected $updateSectionId		    = 357; // tamok - pridano
	protected $viewSectionId		    = 359; // tamok - pridano
	...
	private static $TBODY_LAYOUT = 'data-tbody';
	...
	public function init() {
        ...
        $logger->info('DataSettingsController->init');
	...
	public function indexAction($mode = self::MODE_DEFAULT) {
        ...
        $logger = new Zend_Log(new Zend_Log_Writer_Firebug());
        $logger->info('DataSettingsController->indexAction');
	...
	public function viewAction($mode = self::MODE_DEFAULT) {
		...
		exit;
		$this->_helper->layout->setLayout('empty');
		$params 			= $this->ws->Params->getById( (integer)$this->paramId );
		$params				= It6_ArrayWrapper::toNativeArray($params);
		...
		if ($this->getRequest()->getPost('submit')) {
			...
			if ($form->isValid($data)) {
				...
				if ($this->ws->Params->update($data)) {
				 	...
				 	// ********************
				 	$sql = 'DELETE FROM parametry_opravneni WHERE id_parametr = ' . $this->paramId ;
				    ...
				    foreach ($data as $key=>$value) {
                          ...
                          $privilege = $checkbox[2];
  				 	      ...
  				 	      $sql = 'SELECT * FROM parametry_opravneni WHERE parametry_opravneni.id_parametr = '.$this->paramId.' AND parametry_opravneni.acl_role_id = ' . $acl_role_id;
                          ...
                          if (count($checkquery->fetchAll()) == 0) {
					        $sql = "INSERT INTO parametry_opravneni (id_parametr, acl_role_id, $privilege) 
					                     VALUES ('$this->paramId', '$acl_role_id', '$value')";                          	
                          } else {
					        $sql = "UPDATE parametry_opravneni SET $privilege = '$value' WHERE parametry_opravneni.id_parametr = $this->paramId AND parametry_opravneni.acl_role_id = $acl_role_id";                          	
                          }
    				  	  Zend_Registry::get('zdb_admin')->query($sql);
   				       }
				    }
