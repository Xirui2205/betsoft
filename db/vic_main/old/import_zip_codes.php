...
try {
	...
	$handle = fopen(dirname(__FILE__) . '/import_data/psc.csv', 'r');
while($line = fgetcsv($handle, 1024, ";")) {
        if (strstr($line[0],$line[5]."-") != false){
         echo "skipped\n";
         continue;
        }
        else{
        ...
        $town_part = $array[0];
        $town_part = str_replace('x)', '', $town_part);

                $db->insert(
                        'town_zip_codes',
                                array(
                                                'town_part' => trim($town_part),
                                                'zip_code' => $line[1],
                                                'town' => trim($line[5])
                                                )
                        );
                        echo "INSERTED TOWN:$line[5] TOWN_PART:$town_part WITH ZIP_CODE:$line[1]\n";
        }
        }
	fclose($handle);
