...
CREATE TABLE IF NOT EXISTS `vic_main`.`user_search` (
...
ALTER TABLE  `vic_main`.`user_search` ADD FULLTEXT (
`jmeno` ,
`prijmeni` ,
`nick` ,
`email` ,
...
CREATE TRIGGER `t_uzivatel_after_insert` AFTER INSERT ON `vic_main`.`uzivatel`
 FOR EACH ROW BEGIN
...
CREATE TRIGGER `t_uzivatel_after_update` AFTER UPDATE ON `vic_main`.`uzivatel`
 FOR EACH ROW BEGIN
	UPDATE user_search
...
CREATE TRIGGER `t_uzivatel_after_delete` AFTER DELETE ON `vic_main`.`uzivatel`
 FOR EACH ROW BEGIN
...
BEGIN
	...
	INSERT INTO user_search(user_id, jmeno, prijmeni, nick, email, ulice, telefon, mobil, info, misto) SELECT user_id, jmeno, prijmeni, nick, email, ulice, telefon, mobil, info, misto FROM `vic_main`.`uzivatel`;
END
//
