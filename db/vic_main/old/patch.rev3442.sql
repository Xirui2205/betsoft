DROP VIEW  `ticket_pohled`;
DROP VIEW `ticket_pohled_connect`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ticket_pohled_connect` AS select `a`.`sazka_id` AS `sazka_id`,`a`.`live` AS `live`,`a`.`platna_do` AS `platna_do`,`d`.`stats` AS `stats`,`d`.`stats_user` AS `stats_user`,`d`.`user_id` AS `user_id`,`a`.`proplacena` AS `proplacena`,`a`.`typ_id` AS `typ_id`,`a`.`text` AS `text`,`a`.`vysledek` AS `vysledek`,`a`.`status` AS `status`,`a`.`udalost_id` AS `udalost_id`,`b`.`kurz` AS `kurz`,`b`.`platny_od` AS `platny_od`,`b`.`kurz_zmena` AS `kurz_zmena`,`b`.`sloupec_id` AS `sloupec_id`,`b`.`poradi` AS `poradi`,`c`.`ticket_sazka_zrusena` AS `ticket_sazka_zrusena`,`c`.`ticket_sazka_zrusil_bookmaker_id` AS `ticket_sazka_zrusil_bookmaker_id`,`c`.`ticket_sazka_duvod_zruseni` AS `ticket_sazka_duvod_zruseni`,`c`.`banker` AS `banker`,`c`.`group_id` AS `group_id`,`d`.`ticket_id` AS `ticket_id`,`d`.`handle` AS `ticket_handle`,`d`.`host_id` AS `ticket_branch_id`,`d`.`type` AS `type`,`d`.`win` AS `win`,`d`.`rate` AS `rate`,`d`.`win_real` AS `win_real`,`d`.`rate_real` AS `rate_real`,`d`.`free_bet_bonus` AS `free_bet_bonus`,`d`.`zalozen` AS `zalozen`,`d`.`vyplacen` AS `vyplacen`,`d`.`zruseno` AS `zruseno`,`d`.`is_loss` AS `is_loss`,`d`.`zrusil_bookmaker_id` AS `zrusil_bookmaker_id`,`d`.`duvod_zruseni` AS `duvod_zruseni`,`d`.`castka` AS `castka`,`d`.`system` AS `system`,`d`.`coupon_id` AS `coupon_id`,`d`.`mail` AS `mail`,`d`.`group_count` AS `group_count`,`d`.`group_t` AS `group_t`,`d`.`vyplacen_date` AS `vyplacen_date`,`d`.`vyplacen_bookmaker_id` AS `vyplacen_bookmaker_id`,`d`.`collection_time` AS `collection_time`,`d`.`collection_host_id` AS `collection_host_id`,`d`.`forced_collection_host_id` AS `forced_collection_host_id`,`d`.`cash` AS `cash`,`d`.`point_type_id` AS `point_type_id`,`d`.`rate_advance` AS `rate_advance`,`d`.`forfeit` AS `forfeit`,`d`.`mp` AS `mp`,`d`.`mp_win` AS `mp_win` from (`sazky` `a` join (`sazka_kurz` `b` join (`ticket_kurz` `c` join `ticket` `d` on((`c`.`ticket_id` = `d`.`ticket_id`))) on(((`b`.`sazka_id` = `c`.`sazka_id`) and (`b`.`sloupec_id` = `c`.`sloupec_id`)))) on((`a`.`sazka_id` = `b`.`sazka_id`)));

CREATE ALGORITHM=UNDEFINED
DEFINER=`root`@`localhost`
SQL SECURITY DEFINER VIEW `ticket_pohled` AS SELECT
	...
	`b`.`cash` AS `cash`,
	`b`.`collection_time` AS `collection_time`,
	`b`.`collection_host_id` AS `collection_host_id`,
	`b`.`forced_collection_host_id` AS `forced_collection_host_id`,
	`b`.`point_type_id` AS `point_type_id`,
	`b`.`rate_advance` AS `rate_advance`,
	`b`.`forfeit` AS `forfeit`,
	`b`.`mp` AS `mp`,
	`b`.`mp_win` AS `mp_win`
FROM `ticket_pohled_connect` `b` where (`b`.`platny_od` = (select max(`a`.`platny_od`) AS `maxi_platny_od` from `ticket_pohled_connect` `a` where ((`a`.`zalozen` >= `a`.`platny_od`) and (`a`.`sazka_id` = `b`.`sazka_id`) and (`a`.`ticket_id` = `b`.`ticket_id`)) group by `a`.`ticket_id`,`a`.`sazka_id`));
