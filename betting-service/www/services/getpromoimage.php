...
//echo WEBHOST . "/images/gallery/" . urldecode($_GET["image"]);
if (isset($_GET["image"])) {
    $ftp = new It6_FtpSync_Client();
    $imgName = $ftp->download("/images/gallery/" . $_GET["image"]);

    $img = WideImage_WideImage::load($imgName);
    
    if (!empty($_GET["w"]))
        $img = $img->resize($_GET["w"]);
    $img->output('jpg', 100);
