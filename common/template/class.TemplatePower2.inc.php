...
class TemplatePowerParser
{
  ...
  var $version;
    ...
    * @access private
    */
   function TemplatePowerParser( $tpl_file, $type= T_BYFILE )
   {
       $this->version        = '3.0.2';
...
class TemplatePower2 extends TemplatePowerParser
{
  ...
  var $prepared;
     ...
     * @access public
     */
    function TemplatePower2( $tpl_file='', $type= T_BYFILE )
    {
  ...
  /**********
    ...
    {
        if(!isset($this->parent[$blockname])) return;
        if(!isset($this->index[$this->parent[$blockname]])) return;
     ...
     * @access public
     */
    function assignGlobal( $varname, $value='' , $strict= false ) // strict=f -> konvertovat HTML
    {
        if (is_array( $varname ))
        {
            foreach($varname as $var => $value)
            {
                $this->__assignGlobal( $var, $strict?$value:Help::html($value) );
            }
        }
        else
        {
            $this->__assignGlobal( $varname, $strict?$value:Help::html($value) );
        }
    }
     ...
     * @access public
     */
    function assign( $varname, $value='', $strict = false)
    {
        if (is_array( $varname ))
        {
            foreach($varname as $var => $value)
            {
                $this->__assign( $var, $strict?$value:Help::html($value) );
            }
        }
        else
        {
            $this->__assign( $varname, $strict?$value:Help::html($value) );
        }
    }
    ...
    function gotoBlock( $blockname )
    {
        if ( isset( $this->defBlock[ $blockname ] ) )
        {
               ...
               if(!isset($this->content[ $ind_blockname ])) {
                    $writer = new Zend_Log_Writer_Stream(LOG_DIRECTORY."/tmpl_power.log");
