...
class Zend_Cache_Backend_File extends Zend_Cache_Backend implements Zend_Cache_Backend_ExtendedInterface
{
    ...
    {
        //IT6: BEGIN patch
        if (!isset($options['hashed_directory_umask']) && defined('ZEND_CACHE_FILE_HASHDIR_UMASK'))
            $options['hashed_directory_umask'] = ZEND_CACHE_FILE_HASHDIR_UMASK;
        if (!isset($options['cache_file_umask']) && defined('ZEND_CACHE_FILE_UMASK'))
            $options['cache_file_umask'] = ZEND_CACHE_FILE_UMASK;
        //IT6: END patch
