<?php
class It6_Validate_Int extends Zend_Validate_Int {
...
public static function formatInteger($number) {
	...
	return Zend_Locale_Format::toInteger($number, $options);
//	$locale = localeconv();
//	return number_format($number, $decimals, $locale['decimal_point'], $locale['thousands_sep']);
}


public static function parseInteger($str, $options = array()) {
	try {
		$str = preg_replace('/\\s+/', '', $str);
		if (empty($options['locale']))
			$options['locale'] = self::getAppLocale();
		if(Zend_Locale_Format::isInteger($str, $options))
			return Zend_Locale_Format::getInteger($str, $options);
		else
			return false;
	}
	catch (Exception $e) {
...
public function isValid($value){
	...
	try {
		$int = static::parseInteger($value, array('locale' => $this->getLocale()));
		if (false === $int) {
			$this->_error(parent::NOT_INT);
			return false;
		}
	...
	if (false !== $this->minimum && $int < $this->minimum) {
		$this->_error(self::MSG_MINIMUM);
		return false;
	}

	if (false !== $this->maximum && $int > $this->maximum) {
